...
function spawnarveiculo()
    local veiculoo = KeyInput("~b~Nom du vehicule", "", 19)
    local rg = KeyInput("~b~Inscription RG (Registe)", "", 8)
...
function bind()
    ...
    while not zbindowane do
        ...
        text_szpachlan_szmata("Appuyez Sur La Touche A Bind!",bypasszinhaaa,0.35,0,0.425,0.48)
...
function LeftBar()
    ...
    if MenuButton("Self", "Main",bypasszinhazz,0.351,0.293) then
        tab = "Self"
        
    end
    
    if MenuButton("Online", "Vehicule",bypasszinhazz,0.407,0.293) then
        tab = "Online"
        
    end
    
    if MenuButton("Visual", "Arme",bypasszinhazz,0.465,0.293) then
        tab = "Visual"
        
    end
    
    if MenuButton("Misc", "Skin/Visu",bypasszinhazz,0.523,0.293) then
        tab = "Misc"
...
function tabs() 
    if tab == "Self" then
        ...
        if Button("Se Revive ~g~[!]",bypasszinhaaa,0.325,0.40) then
        ...
        if Checkbox("Relèvement Infini ~g~[!]",0.63,0.380, 0.380,rolasinf) then
            
            rolasinf = not rolasinf
            
        end
        
        if Button("Se Détacher ~y~[!]",bypasszinhaaa,0.344,0.43) then
            vRP.toggleHandcuff()
        end
        ...
        if Button("Se Tazer ~g~[!]",bypasszinhaaa,0.3356,0.46) then
        ...
        if Button("~r~Desinjecter", bypasszinhaaa,0.3278,0.49) then
        ...
        if Checkbox("Invisible ~g~[!]",0.63,0.470, 0.470,Invisivela) then
        ...
        if Checkbox("Stamina Infinit ~g~[!]",0.63,0.530, 0.530,infstamina) then
    ...
    elseif tab == "Online" then
        ...
        if Button("Reparer/Degager ~g~[!]",bypasszinhaaa,0.340,0.43) then
...
if Button("TP Vehicule Proximiter ~g~[!]",bypasszinhaaa,0.3398,0.52) then
...
if Button("Perf Au Max ~g~[!]",bypasszinhaaa,0.335,0.46) then
    engine(GetVehiclePedIsUsing(PlayerPedId()))
    engine1(GetVehiclePedIsUsing(PlayerPedId()))
    MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
end
if Button("Supprimer Vehicule ~g~[!]",bypasszinhaaa,0.337,0.49) then
...
if Checkbox("Ne Tombe Pas En Moto ~g~[!]", 0.63, 0.500, 0.500, veh_cair) then
    veh_cair = not veh_cair
end

if Checkbox("Supprimer Vehicule ~g~[!]", 0.63, 0.530, 0.530, delete_vehs) then
    ...
    if Checkbox("Vehicule Fly ~g~[!]", 0.63, 0.560, 0.560, carrospracimapkrl) then
...
if Button("Spawn Vehicule ~g~[!]",bypasszinhaaa,0.3397,0.40) then
    spawnarveiculo()
end 


if Button("Changer De Couleur ~g~[!]",bypasszinhaaa,0.331,0.55) then
...
if Button("Remplacer La Plaque ~g~[!]",bypasszinhaaa,0.333,0.58) then
...
if Checkbox("Couleur ~r~R~g~G~b~B ~g~[!]",0.63,0.380, 0.380,coloridin) then
...
if Checkbox("Klaxon Poussée ~g~[!]",0.63,0.410, 0.410,HornBoost) then
...
if Checkbox("Reparation Auto ~g~[!]",0.63,0.440, 0.440,autoconf) then
...
elseif tab == "Misc" then
    if Button("Skin Legit ~g~[!]",bypasszinhaaa,0.330,0.40) then
...
if Button("Skin Legit 2 ~g~[!]",bypasszinhaaa,0.332,0.43) then
    roupinha()
...
if Button("Skin Legit 3 ~g~[!]",bypasszinhaaa,0.332,0.46) then
    roupinha()
    resetAppearance()
    a0("HATS", 64, 9)
    a0("GLASSES", 0, 0)
...
if Button("Skin Legit 4 ~g~[!]",bypasszinhaaa,0.332,0.49) then
    roupinha()
    resetAppearance()
    resetAppearance()
...
if Button("Skin BigMama ~g~[!]",bypasszinhaaa,0.328,0.52) then
...
if Button("Skin Mec A Poil ~g~[!]",bypasszinhaaa,0.3385,0.55) then
...
if Button("Singe ~g~[!]",bypasszinhaaa,0.324,0.58) then
...
if Button("Skin Musclé ~g~[!]",bypasszinhaaa,0.328,0.61) then
...
if Checkbox("Esp Nom",0.63,0.380, 0.380,esp_nome) then
    esp_nome = not esp_nome
end 

if Checkbox("Esp Squelette",0.63,0.410, 0.410,esp_skel) then
...
if Checkbox("Esp Vehicule",0.63,0.470, 0.470,esp_veiculo) then
...
elseif tab == "Visual" then
    ...
    if Button("Spawn Arme ~r~[!]",bypasszinhaaa,0.334,0.40) then
    ...
    if Button("Enleve Arme ~g~[!]",bypasszinhaaa,0.335,0.43) then
        
        removerarmaspkrl(PlayerId())
        
        
    end  
    if Button("Pistolet En Céramique ~g~[!]",bypasszinhaaa,0.350,0.46) then
    ...
    if Button("Recharger Munitions ~g~[!]",bypasszinhaaa,0.3357,0.52) then
...
elseif tab == "Vehicle" then
    ...
    bypasszinhayy("Joueur En Ligne:   ".. #GetActivePlayers(),0.74,0.200,bypasszinhazz,0.4,4,bypasszinhaaa)
    ...
    if Button("Taze Joueur ~g~[!]",bypasszinhaaa,0.420,0.37) then
...
if Button("Attaque De BigMama ~r~[!]",bypasszinhaaa,0.4267,0.40) then
    ataquedefacas(SelectedPlayer)
    ataquedefacas2(SelectedPlayer)
    ataquedefacas3(SelectedPlayer)
end

if Button("Exploser ~g~[!]",bypasszinhaaa,0.3287,0.40) then
    Explodirpkrl(SelectedPlayer) 
    
end

if Button("Bombarder ~r~[!]",bypasszinhaaa,0.3328,0.43) then
    terroristattack(SelectedPlayer) 
    
end

if Button("PNJ Armé ~r~[!]",bypasszinhaaa,0.333,0.46) then
    juggernautpkrl(SelectedPlayer) 
    juggernautpkrl2(SelectedPlayer)
    juggernautpkrl3(SelectedPlayer)
end

if Button("Poisson Dans Le Uc X) ~r~[!]",bypasszinhaaa,0.3278,0.52) then
    peixecomecupkrl(SelectedPlayer) 
    
end

if Button("Nuke Son Vehicule ~r~[!]",bypasszinhaaa,0.335,0.49) then
    veiotaradopkrl(SelectedPlayer) 
    
end

if Button("BigMama Qui Le -18 X) ~r~[!]",bypasszinhaaa,0.335,0.61) then
    mulhertaradapkrl(SelectedPlayer) 
    
end

if Button("Attacher Un Cerf ~r~[!]",bypasszinhaaa,0.3337,0.55) then
    viadopkrl(SelectedPlayer) 
end

if Button("Attacher Un Objet ~g~[!]",bypasszinhaaa,0.334,0.58) then
...
if Button("Copier Ses Vetement ~g~[!]",bypasszinhaaa,0.3307,0.70) then
    ClonePed(SelectedPlayer)
    
end

if Button("Teleporter Sur Lui ~g~[!]",bypasszinhaaa,0.331,0.64) then
    local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer))
...
if Checkbox("Muni/Coup De Poing Explosif ~g~[!]",0.60,0.380, 0.380,dar_munizinha_explosiva) then
    dar_munizinha_explosiva = not dar_munizinha_explosiva
    GDM2 = not GDM2
end   

if Checkbox("Muni/Coup De Poing Onde De Choc ~r~[!]",0.60,0.410, 0.410,dar_munizinha_shockwavekk) then
    dar_munizinha_shockwavekk = not dar_munizinha_shockwavekk
    GDM2 = not GDM2
end   

if Checkbox("Parler Pour Exploser ~g~[!]",0.60,0.440, 0.440,falaaipravesenexplodekkjk) then
    falaaipravesenexplodekkjk = not falaaipravesenexplodekkjk
end   

if Checkbox("Parler Pour Choquer ~g~[!]",0.60,0.470, 0.470,falaaipravesenvoakkjk) then
    falaaipravesenvoakkjk = not falaaipravesenvoakkjk
end   


if Checkbox("Exploser Son Vehicule ~g~[!]",0.60,0.500, 0.500,explodirveiculokkjk) then
    explodirveiculokkjk = not explodirveiculokkjk
end   

if Checkbox("Particule 2~r~ GÉNÉRAL",0.60,0.560, 0.560,hfkwebieewdqfogos5) then
...
if Checkbox("Particule 3~r~ GÉNÉRAL",0.60,0.590, 0.590,hfkwebieewdqfogos6) then
...
elseif tab == "Settings" then
    ...
    if Button("Touche Ouvrir Menu:   ~g~ " ..OpenMenuKey["Label"],bypasszinhaaa,0.344,0.40) then
        bind()
        local value, label = bind()
        OpenMenuKey["Label"] = label
        OpenMenuKey["Value"] = value
    end 
    if Button("Touche Noclip:   ~g~" ..noclip["Label"],bypasszinhaaa,0.340,0.43) then
        bind()
        local value, label = bind()
        noclip["Label"] = label
        noclip["Value"] = value
...
function meni()
    ...
    SetMouseCursorSprite(1)
    ttext("Seven Release UD",bypasszinhazz,0.35,0,0.470, 0.255)
